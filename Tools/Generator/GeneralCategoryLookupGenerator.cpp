...
static const string DATA_ARRAY_NAME = "PrimaryGeneralCategoryData";

static const string MAIN_INDEXES_ARRAY_TYPE = "static const SBUInt16";
static const string MAIN_INDEXES_ARRAY_NAME = "MainGeneralCategoryIndexes";

static const string BRANCH_INDEXES_ARRAY_TYPE = "static const SBUInt16";
static const string BRANCH_INDEXES_ARRAY_NAME = "BranchGeneralCategoryIndexes";
...
void GeneralCategoryLookupGenerator::displayGeneralCategoriesFrequency() {
    ...
    for (uint32_t codePoint = 0; codePoint <= last; codePoint++) {
        frequency[m_derivedGeneralCategory.generalCategoryOf(codePoint)] += 1;
    }
...
void GeneralCategoryLookupGenerator::collectMainData() {
    auto lastCodePoint = m_derivedGeneralCategory.lastCodePoint();
    ...
    for (size_t i = 0; i < maxSegments; i++) {
        ...
        for (uint32_t codePoint = segmentStart; codePoint <= segmentEnd; codePoint++) {
            dataset->push_back(&m_derivedGeneralCategory.generalCategoryOf(codePoint));
        }
...
void GeneralCategoryLookupGenerator::generateFile(const std::string &directory) {
    ...
    FileBuilder source(directory + "/GeneralCategoryLookup.c");
    ...
    source.newLine();
    source.appendTab().append("return Cn;").newLine();
    source.append("}").newLine();
    source.newLine();

    for (auto cn : categoryNames) {
        source.append("#undef " + cn).newLine();
    }
}
