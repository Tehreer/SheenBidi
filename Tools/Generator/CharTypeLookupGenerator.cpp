...
static const string DATA_ARRAY_NAME = "__SBCharTypePrimaryData";

static const string MAIN_INDEXES_ARRAY_TYPE = "static const SBUInt16";
static const string MAIN_INDEXES_ARRAY_NAME = "__SBCharTypeMainIndexes";

static const string BRANCH_INDEXES_ARRAY_TYPE = "static const SBUInt16";
static const string BRANCH_INDEXES_ARRAY_NAME = "__SBCharTypeBranchIndexes";
...
void CharTypeLookupGenerator::generateFile(const std::string &directory) {
    ...
    for (; dataPtr != dataEnd; dataPtr++) {
        ...
        for (size_t j = 0; j < length; j++) {
            arrData.appendElement(m_bidiClassDetector.numberToName(segment.dataset->at(j)));
    ...
    FileBuilder file(directory + "/SBCharTypeLookup.c");

    file.append(arrData);
    file.newLine();
    
    file.append(arrMainIndexes);
    file.newLine();

    file.append(arrBranchIndexes);
    file.newLine();
}
